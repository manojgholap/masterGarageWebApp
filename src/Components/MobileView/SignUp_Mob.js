...
const SignUp_Mob = () => {
  ...
  const handleclick = () => {
    if (!phone || phone.length < 12) {
      ...
      axios.post(process.env.REACT_APP_Master_Garage + "user/signUp.php", { mobileNumber: phone.substring(2, 12),action:"signup" }).then((res, err) => {
  ...
  return (<>
    ...
    {isotp ? <Otp_Mob mobileNumber={phone} ownerId={ownerId}></Otp_Mob> : 
    <> 
    <div>
      ...
      <div className="login_first">
        ...
        <div>
          ...
          />
          <Link  onClick={()=>handleclick()}><button className='btn_login'>CONTINUE</button>  </Link>
          <p>Allready Account? <Link to="/Login2_Mob">Create Account</Link></p>
        </div>
      </div></>}
  </>
  )
